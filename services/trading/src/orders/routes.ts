...
export const orderRoutes: FastifyPluginAsyncTypebox<OrderRoutesOptions> = (app, opts) => {
  ...
  app.post(
    '/api/v1/orders',
    { preHandler: requireAuth, schema: { body: OrderBody } },
    async (request, reply) => {
      const accountId = request.account.id;
      const orderId = randomUUID();
      const trades = await listAccountTrades(accountId);
      const decision = placeOrder(request.body, {
        lastPrice,
        trades,
        now: Date.now(),
        orderId,
      ...
      if (!decision.ok) {
        await appendEvents(accountId, [
          {
            streamId: `order:${orderId}`,
            type: EVENT_TYPES.orderRejected,
  ...
  app.get(
    '/api/v1/orders/:id',
    { preHandler: requireAuth, schema: { params: Type.Object({ id: Type.String() }) } },
    async (request) => ({
      orderId: request.params.id,
      events: await listOrderEvents(request.params.id, request.account.id),
    }),
  );
